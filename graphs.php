...
function graphposts($thresholdneg, $thresholdpos, $courseid) {
    ...
    $output .= html_writer::tag('span',  get_string('avg', 'local_survey_intelligence'));
    $htmlcontent = '<div class="no-overflow">
        <b>'.get_string('avg', 'local_survey_intelligence').': </b>'.get_string('avg_des', 'local_survey_intelligence').'<br>
        </div>';
    ...
    get_string('avg_all', 'local_survey_intelligence') . substr($meanall->mean, 0, 7),
    ['class' => 'h5 col-6']);
    ...
    if (!is_null($courseid) || $courseid != 0) {
        ...
        if (!is_null($meanall->mean)) {
            $output .= html_writer::tag('div',
            get_string('avg_course', 'local_survey_intelligence') . substr($meanall->mean, 0, 7),
            ['class' => 'h5 col-6']);
        }
    }
    ...
    $output .= html_writer::tag('div', get_string('Graphs', 'local_survey_intelligence'), ['class' => 'h3 m-5 border-bottom ']);

    $rd = get_post_from_course($courseid);

    $llengues = array('ca' => 0, 'es' => 0, 'en' => 0, get_string('Others', 'local_survey_intelligence') => 0);
    $sentiment = array('pos' => 0, 'neu' => 0, 'neg' => 0, 'error' => 0);
    foreach ($rd as $key => $value) {
        $llengues[(array_key_exists($value->language, $llengues))
        ? $value->language
        : get_string('Others', 'local_survey_intelligence')] += 1;
        $num = $value->polarity;
    ...
    $leng = new core\chart_series(get_string('Languages', 'local_survey_intelligence'), array_values($llengues));
    $chart->set_title(get_string('Languages', 'local_survey_intelligence'));
    ...
    $chart = new \core\chart_pie();
    $sen = new core\chart_series(get_string('Sentiment', 'local_survey_intelligence'), array_values($sentiment));
    $chart->set_title(get_string('Sentiment', 'local_survey_intelligence'));
    $chart->add_series($sen); // On pie charts we just need to set one series.
    $chart->set_labels([get_string('pos', 'local_survey_intelligence'), get_string('neu', 'local_survey_intelligence'),
        get_string('neg', 'local_survey_intelligence'), get_string('err', 'local_survey_intelligence')]);
    $output .= $OUTPUT->render($chart);
